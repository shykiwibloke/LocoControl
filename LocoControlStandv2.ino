  ...
  else
  {
    ...
    else if (gDirection != DIR_NONE)
    {
      ...
      else if (gThrottleNotch > 0)
      {
        ...
        //Send Sound Command immediately
        Serial.print(F("S:"));
        Serial.print(gThrottleNotch);
        Serial.println(F(":"));

        if (old == 0 )   //if last status was IDLE and we are accelerating - delay the actual motor pickup for a few
        { //seconds to get the throttle up-before-you-move effect.
          gtimer.setTimeout(3000, SetMotorSpeed);
        }
        else
          SetMotorSpeed();   //go adjust immediately
...
void SetMotorSpeed(void)
{
  ...
  int speed = MOTOR_NOTCH * gThrottleNotch;        //used to scale the provided notch to a speed setting the controller understands (-2047-0-2047 range)
...
Serial.println(count);
  if (--count == 0)
  {
    //todo - restore both ditch lights to whatever they were
    Serial.println(F("L:4:Ditch Flashing Stopped"));
    count = 21;
    return;
  }

  if (count%2 == 0)     //if count is even
    Serial.println(F("L:4:Ditch Flash Left"));
  else
    Serial.println(F("L:4:Ditch Flash Right"));
...
void CheckHorn(void)
{
  ...
  if (newval[0] == 0 && newval[1] == 0 && newval[2] == 0 && gHorn == 0)	//Horn button pressed, we have not seen this yet
  {
    ...
    gtimer.setTimer(500, FlashDitchlights, 21);
