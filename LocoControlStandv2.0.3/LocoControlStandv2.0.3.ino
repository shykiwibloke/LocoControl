  ...
  else
  {
    ...
    else if (gDirection != DIR_NONE)
    {
      ...
      else if (gThrottleNotch > 0)
      {
        ...
        //if last status was IDLE and we are accelerating AND we have momentum wound up past half 
		    //- delay the actual motor pickup for a few seconds so the motor revs sound builds before we move 
		    if (old == STATUS_IDLE && gMomentum > 1024)   
...
void DoTimedIntervalChecks(void)
{
  ...
  {
    CalcMomentum();    //calc every second run, but opp motor current call
  }
