 ...
 *  4.1.2
27/05/2019: Minor bug in reverse with dynamic mode fixed.
 *	4.1.0
...
void loop(void)
{
  ...
  else 
  {
		...
		GetMotorAmps();             //fetch and send one motor's current per call, and battery voltage once per 6 motor checks.
...
void GetMotorAmps()
{
	//This is the temporary hack version of the old code below
	
	//thinking. 
	
	//Fetch the max positive and max negative values for each motor.
  ...
  if(gControlState != CONTROL_STATE_ERROR
     ...
     && gControlsChanged == false)  //Only request from motor drivers if comms channel has the buffer space to cope 
																		...
																		//AND status is something that can use the current measurements (not Error or Id																		{
	{
...
controlState_t EvaluateLevers(void)
{
	...
	controlState_t newState = CONTROL_STATE_UNDEFINED;
		...
		if(gDynamicEnabled)  //if dynamic mode is permitted
		{
			...
			else if(gCurrentDynNotch == 0 && gCurrentThrNotch == 0)  
			{			
				if(gDirection == DIR_FWD && gCurrentMotorSpeed > 0)  //only go to coast if in forward direction - no dynamic in reverse
				{
			...
			{
				if(gDirection == DIR_REV)
				{
					newState = CONTROL_STATE_POWER;
				}	
				else if (CalcSpeedFromNotch(gCurrentThrNotch) > gCurrentMotorSpeed)   //Throttle is above whatever speed we are currently
				{
						newState = CONTROL_STATE_POWER;	
				}
				else
